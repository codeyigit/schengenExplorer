package hr.algebra.schengenexplorer.dao
...
private val CREATE_TABLE = "create table $TABLE_NAME( " +
        ...
        "${Item::flagDescription.name} text not null, " +
        "${Item::officialName.name} text not null, " +
        "${Item::picturePath.name} text not null, " +
        "${Item::read.name} integer not null" +
        ")"
private const val DROP_TABLE = "drop table $TABLE_NAME"

class SchengenSqlHelper(context: Context?) :SQLiteOpenHelper(
    ...
    override fun insert(values: ContentValues?) = writableDatabase
